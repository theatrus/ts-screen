...
pub fn read_fits_metadata(path: &Path) -> Result<FitsMetadata> {
    ...
    if let Some(naxis) = keywords.get("NAXIS").and_then(|s| s.parse::<usize>().ok()) {
        if naxis > 0 {
            ...
            for i in 1..=naxis {
                if let Some(dim) = keywords.get(&format!("NAXIS{}", i))
            ...
            if dimensions.len() >= 2 {
                ...
                let bit_depth = keywords.get("BITPIX")
...
pub fn format_fits_metadata(metadata: &FitsMetadata, verbose: bool) -> String {
    ...
    if let Some(median) = metadata.primary_header.get("MEDIAN")
        .or_else(|| metadata.primary_header.get("MEDIANPX")) {
        output.push_str(&format!("  Median: {}\n", median));
    }
    if let Some(mean) = metadata.primary_header.get("MEAN")
        .or_else(|| metadata.primary_header.get("MEANPX")) {
        output.push_str(&format!("  Mean: {}\n", mean));
    }
    if let Some(stddev) = metadata.primary_header.get("STDEV")
    ...
    // Position info
    if let Some(ra) = metadata.primary_header.get("OBJCTRA").or(metadata.primary_header.get("RA")) {
        if let Some(dec) = metadata.primary_header.get("OBJCTDEC").or(metadata.primary_header.get("DEC")) {
