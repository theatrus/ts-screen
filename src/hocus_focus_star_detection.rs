...
pub fn detect_stars_hocus_focus(
    ...
    let noise_estimate = kappa_sigma_noise_estimate(&structure_map, width, height, params.noise_clipping_multiplier);
    
    // Debug output
    eprintln!("Debug HocusFocus: noise_sigma: {:.3}, background_mean: {:.3}", 
    ...
    eprintln!("Debug HocusFocus: median: {:.3}, threshold: {:.3}", median, threshold);
    ...
    eprintln!("Debug HocusFocus: Binary map has {} non-zero pixels ({:.2}%)", 
              non_zero, non_zero as f64 / binary_map.len() as f64 * 100.0);
    
    // Apply erosion to break up connected components
    if non_zero > structure_map.len() / 100 {  // If more than 1% of pixels are set
        binary_map = apply_erosion(&binary_map, width, height);
        let eroded_count = binary_map.iter().filter(|&&x| x).count();
        eprintln!("Debug HocusFocus: After erosion: {} non-zero pixels ({:.2}%)", 
                  eroded_count, eroded_count as f64 / binary_map.len() as f64 * 100.0);
    }
    
    // Step 6: Find star candidates
    let candidates = find_star_candidates(&binary_map, width, height, params);
    eprintln!("Debug HocusFocus: Found {} star candidates", candidates.len());
    
    // Step 7: Measure and validate stars
    let stars = measure_stars(&working_data, width, height, candidates, params, &noise_estimate);
...
fn apply_hotpixel_filter(data: &[u16], width: usize, height: usize, threshold_percent: f64) -> Vec<u16> {
...
fn create_structure_map(data: &[u16], width: usize, height: usize, params: &HocusFocusParams) -> Vec<f64> {
    ...
    let max = structure_map.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    ...
    eprintln!("Debug structure_map: min={:.1}, max={:.1}, non_zero={} ({:.1}%)", 
              min, max, non_zero, non_zero as f64 / structure_map.len() as f64 * 100.0);
    eprintln!("  Above 10: {} ({:.1}%), Above 50: {} ({:.1}%), Above 100: {} ({:.1}%)",
              above_10, above_10 as f64 / structure_map.len() as f64 * 100.0,
              above_50, above_50 as f64 / structure_map.len() as f64 * 100.0,
              above_100, above_100 as f64 / structure_map.len() as f64 * 100.0);
...
fn kappa_sigma_noise_estimate(data: &[f64], _width: usize, _height: usize, clipping_multiplier: f64) -> KappaSigmaResult {
...
fn find_star_candidates(
    ...
    for y_top in 0..(height - 1) {
        for x_left in 0..(width - 1) {
            ...
            if star_bounds.2 < params.min_star_size || star_bounds.3 < params.min_star_size {
                too_small += 1;
                eprintln!("  Structure too small: {}x{} at ({},{})", star_bounds.2, star_bounds.3, star_bounds.0, star_bounds.1);
            ...
            if star_bounds.2 > params.max_star_size || star_bounds.3 > params.max_star_size {
                too_large += 1;
                eprintln!("  Structure too large: {}x{} at ({},{})", star_bounds.2, star_bounds.3, star_bounds.0, star_bounds.1);
                // Still need to clear to avoid re-processing
                for sy in star_bounds.1..(star_bounds.1 + star_bounds.3).min(height) {
                    for sx in star_bounds.0..(star_bounds.0 + star_bounds.2).min(width) {
                        structure_map[sy * width + sx] = false;
            ...
            // Calculate centroid
            let center_x = star_pixels.iter().map(|&(x, _)| x as f64).sum::<f64>() / star_pixels.len() as f64;
            let center_y = star_pixels.iter().map(|&(_, y)| y as f64).sum::<f64>() / star_pixels.len() as f64;
    ...
    eprintln!("Debug star scanning: total_structures={}, too_small={}, too_large={}, candidates={}", 
              total_structures, too_small, too_large, candidates.len());
...
fn measure_stars(
    ...
    for candidate in candidates {
        ...
        if !validate_star(&candidate, peak, median, background, hfr, snr, params, width, height) {
            continue;
        }
    ...
    let background = if !background_pixels.is_empty() {
        if background_pixels.len() % 2 == 0 {
            (background_pixels[background_pixels.len() / 2 - 1] + 
             background_pixels[background_pixels.len() / 2]) / 2.0
        } else {
    ...
    let star_median = if !star_pixel_values.is_empty() {
        if star_pixel_values.len() % 2 == 0 {
            (star_pixel_values[star_pixel_values.len() / 2 - 1] + 
             star_pixel_values[star_pixel_values.len() / 2]) / 2.0
        } else {
