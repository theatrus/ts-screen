...
#[cfg(test)]
mod tests {
    use crate::nina_star_detection::{detect_stars_with_original, StarDetectionParams, StarSensitivity, NoiseReduction};
    use rand::prelude::*;
    ...
    fn detect_stars_with_stretching(image: &SyntheticImage, params: &StarDetectionParams) -> crate::nina_star_detection::StarDetectionResult {
        ...
        fn add_gaussian_star(&mut self, x: f64, y: f64, fwhm: f64, peak_brightness: u16) {
            ...
            for py in y_min..y_max {
                for px in x_min..x_max {
                    ...
                    // Gaussian profile
                    let intensity = peak_brightness as f64 * (-distance_sq / (2.0 * sigma * sigma)).exp();
    ...
    fn test_single_bright_star() {
        ...
        println!("Single star test - Detected: {}, Expected: 1", result.star_list.len());
        ...
        let position_error = ((detected.position.0 - expected.x).powi(2) + 
                             (detected.position.1 - expected.y).powi(2)).sqrt();
        
        println!("Position error: {:.2} pixels", position_error);
        assert!(position_error < 2.0, "Star position should be within 2 pixels");
        ...
        println!("HFR - Detected: {:.2}, Expected: {:.2}, Error: {:.1}%", 
                 detected.hfr, expected_hfr, hfr_error * 100.0);
    ...
    fn test_multiple_stars_different_brightness() {
        ...
        println!("Multiple stars test - Detected: {}, Expected: 3-4", result.star_list.len());
        println!("  Detected stars HFR: {:?}", 
                 result.star_list.iter().map(|s| format!("{:.2}", s.hfr)).collect::<Vec<_>>());
        assert!(result.star_list.len() >= 3, "Should detect at least 3 bright stars");
        assert!(result.star_list.len() <= 4, "Should not detect more than 4 stars");
    }
    ...
    fn test_star_field_with_noise() {
        ...
        for sensitivity in [StarSensitivity::Normal, StarSensitivity::High] {
            ...
            println!("Star field {:?} - Detected: {}, Expected: ~{}", 
                     sensitivity, result.star_list.len(), num_stars);
            
            // Should detect most stars but not all (some may be too faint)
            assert!(result.star_list.len() >= num_stars * 60 / 100, 
                    "Should detect at least 60% of stars");
            assert!(result.star_list.len() <= num_stars * 120 / 100,
    ...
    fn test_circular_vs_gaussian_stars() {
        ...
        println!("Shape test - Detected {} stars (2 expected)", result.star_list.len());
        ...
        if result.star_list.len() == 2 {
            ...
            println!("Circular star HFR: {:.2}, Gaussian star HFR: {:.2}", 
                     hfr1, hfr2);
        }
    }
    ...
    fn test_close_stars_separation() {
        ...
        assert!(result.star_list.len() >= 1, "Should detect at least one star");
    ...
    fn test_edge_stars() {
        ...
        println!("Edge stars test - Detected: {} (5 expected, some edge stars may be filtered)", 
                 result.star_list.len());
        assert!(result.star_list.len() >= 1, "Should detect at least the center star");
        assert!(result.star_list.len() <= 5, "Should not detect more than 5 stars");
    }
    ...
    fn test_noise_reduction_effect() {
        ...
        println!("Noise reduction - Without: {} stars, With: {} stars", 
                 result_no_nr.star_list.len(), result_with_nr.star_list.len());
        
        // With noise reduction should detect similar or fewer false positives
        assert!(result_with_nr.star_list.len() <= result_no_nr.star_list.len() + 2,
