...
impl WaveletStructureRemover {
    ...
    fn remove_structures_atrous(&self, data: &[f64], width: usize, height: usize) -> Result<Vec<f64>> {
    ...
    pub fn remove_structures_multi_method(&self, data: &[f64], width: usize, height: usize) -> Result<Vec<f64>> {
    ...
    pub fn preprocess_with_edge_preserving(&self, data: &[f64], _width: usize, _height: usize) -> Result<Vec<f64>> {
...
mod tests {
    ...
    fn test_multi_method_structure_removal() {
        ...
        let data: Vec<f64> = (0..64).map(|i| {
            ...
            let detail = if (x == 3 && y == 3) || (x == 6 && y == 5) { 50.0 } else { 0.0 };
            structure + detail
        }).collect();
