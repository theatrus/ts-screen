...
fn process_file_movement(
    ...
    // Build the paths
    let (source_path, dest_path, actual_source_path) = build_file_paths(
        base_dir, 
        &path_parts, 
        filename
    )?;
    ...
    if dry_run {
        ...
        match fs::rename(&actual_source_path, &dest_path) {
            ...
            Err(e) => {
                return Err(anyhow::anyhow!("Failed to move {}: {}", 
                    actual_source_path.display(), e));
            }
        }
    }
...
fn build_file_paths(
    ...
    if actual_source_path.is_none() && source_path.parent().is_some() {
        let rejected_path = source_path.parent().unwrap().join("rejected").join(filename);
        if rejected_path.exists() {
            actual_source_path = Some(rejected_path);
        }
    }
...
fn extract_target_and_date<'a>(path_parts: &[&'a str], filename: &'a str) -> Option<(&'a str, &'a str)> {
