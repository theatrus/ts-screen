...
use super::star_selection::{select_stars, SelectionStrategy, SortMetric};
...
pub fn visualize_psf_multi(
    ...
    if verbose {
        eprintln!(
            "Detecting stars with PSF fitting enabled ({:?})...",
            psf_type_enum
        );
    }
    ...
    let grid_cols = if selection_mode == "corners" && grid_cols != 3 {
        3
    } else {
        grid_size
    };
