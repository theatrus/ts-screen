/// Benchmark comparing PSF fitting vs simple HFR calculation
/// 
/// Usage: cargo run --release --bin benchmark_psf -- <fits_path>

use anyhow::Result;
use psf_guard::hocus_focus_star_detection::{detect_stars_hocus_focus, HocusFocusParams};
use psf_guard::image_analysis::FitsImage;
use psf_guard::psf_fitting::PSFType;
use std::env;
use std::path::Path;
...
fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        eprintln!("Usage: {} <fits_path>", args[0]);
        std::process::exit(1);
    }
    
    let fits_path = &args[1];
    ...
    for (name, psf_type) in &configs {
        ...
        for _ in 0..n_runs {
            ...
            if !result.stars.is_empty() {
                ...
                if *psf_type != PSFType::None {
                    psf_success_count = result.stars.iter()
        ...
        println!("{:<20} | {:>10} | {:>10.3}s | {:>7.2}ms | {:>10.3} | {:>10.3}", 
                 name, star_count, avg_time, time_per_star, avg_hfr, hfr_std);
        
        if *psf_type != PSFType::None && star_count > 0 {
            println!("{:<20} | PSF fits succeeded: {}/{} ({:.1}%)", 
                     "", psf_success_count, star_count, 
    ...
    for psf_type in &[PSFType::Gaussian, PSFType::Moffat4] {
        ...
        if !stars_with_psf.is_empty() {
            ...
            // Collect R² values
            let r_squared_values: Vec<f64> = stars_with_psf.iter()
                .map(|s| s.psf_model.as_ref().unwrap().r_squared)
                .collect();
            
            let avg_r2 = r_squared_values.iter().sum::<f64>() / r_squared_values.len() as f64;
            let min_r2 = r_squared_values.iter().fold(f64::INFINITY, |a, &b| a.min(b));
            let max_r2 = r_squared_values.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
            
            // Collect FWHM values
            let fwhm_values: Vec<f64> = stars_with_psf.iter()
                .map(|s| s.psf_model.as_ref().unwrap().fwhm)
                .collect();
            
            let avg_fwhm = fwhm_values.iter().sum::<f64>() / fwhm_values.len() as f64;
            let fwhm_std = {
                let variance = fwhm_values.iter()
                    .map(|&f| (f - avg_fwhm).powi(2))
                    .sum::<f64>() / fwhm_values.len() as f64;
                variance.sqrt()
            };
            
            println!("  R² Statistics: avg={:.3}, min={:.3}, max={:.3}", avg_r2, min_r2, max_r2);
            ...
            for (i, star) in sorted_by_r2.iter().take(5).enumerate() {
                ...
                println!("    {}. Position ({:.1}, {:.1}): R²={:.3}, FWHM={:.2}, Ecc={:.3}",
                         i + 1, star.position.0, star.position.1,
                         psf.r_squared, psf.fwhm, psf.eccentricity);
            }
            println!();
        }
    }
