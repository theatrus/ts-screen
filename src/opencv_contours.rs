...
impl OpenCVBlobDetector {
    ...
    pub fn analyze_star_contours(&self, binary_image: &[u8], width: usize, height: usize) -> Result<Vec<StarContour>> {
        ...
        stars.sort_by(|a, b| b.area.partial_cmp(&a.area).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub fn assess_star_quality(&self, contour: &StarContour) -> f64 {
        ...
        (circularity_score * 0.4 + convexity_score * 0.3 + aspect_score * 0.2 + area_score * 0.1).min(1.0)
    }
}
...
impl OpenCVBlobDetector {
    ...
    pub fn analyze_star_contours(&self, binary_image: &[u8], width: usize, height: usize) -> Result<Vec<StarContour>> {
        ...
        let star_contours: Vec<StarContour> = blobs.into_iter().map(|blob| {
...
mod tests {
    ...
    fn test_star_quality_assessment() {
        ...
        let star = StarContour {
            ...
            circularity: 0.8,
            convexity: 0.9,
            bounding_rect: Rectangle { x: 10, y: 10, width: 10, height: 10 },
            centroid: (15.0, 15.0),
        };
